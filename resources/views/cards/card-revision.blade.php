<x-app :deck="$card?->deck">
    ...
    <div class="flex flex-col justify-center items-center w-full h-full p-1" x-data="{flip: false}">
        ...
        @if($card)
            ...
            <div class="flex gap-2 border-b border-gray-400 pb-3">
                <button @click="flip = !flip"  class="flex gap-2 items-center bg-green-700 rounded-lg px-4 py-2 text-white disabled:bg-gray-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    Ver <span x-show="!flip">Resposta</span> <span x-show="flip">Pergunta</span>
                </button>
                

                <a href="{{route('cards.edit', $card->id)}}" class="flex gap-2 items-center bg-gray-700 rounded-lg px-4 py-2 text-white disabled:bg-gray-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
